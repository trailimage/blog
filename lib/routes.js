...
admin.get('/', c.admin.home);
admin.post('/view/delete', c.cache.deletePost);
admin.post('/map/delete', c.cache.deleteMap);
...
function standard(app) {
   ...
   // TODO make handler for photo ID viewer
   ...
   app.get(`/${postKey}/map`, c.map.forPost);
   app.get(`/${postKey}/gpx`, c.map.download);
   app.get(`/${postKey}/map/${photoID}`, c.map.forPost);
