...
class ElementBase {
	...
	constructor(style) {
		...
		this.minHeight = NaN;
		/** @type String */
		this.alignContent = TI.PDF.Align.Inherit;
		/** @type String */
		this.verticalAlign = TI.PDF.Align.Inherit;
		/**
		 ...
		 * @alias TI.PDF.Element.Base.style
		 * @type String
		 */
		this.style = style;
	...
	/**
	 * Position elements that are anchored to only one edge and applies alignment
	 * @param {TI.PDF.Element.Area} area Container area
	 * @param {String} [alignHorizontal]
	 * @param {String} [alignVertical]
	 */
	positionWithin(area, alignHorizontal, alignVertical) {
		...
		if (alignHorizontal !== undefined) {
			switch (alignHorizontal) {
				case TI.PDF.Align.Center: this.horizontalCenter(area.width); break;
			}
		}

		if (alignVertical !== undefined) {
			switch (alignVertical) {
				case TI.PDF.Align.Middle: this.verticalCenter(area.height); break;
			}
		}
	}

	/**
	 * Apply style rules and add parent dimensions
