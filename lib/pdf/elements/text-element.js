...
class TextElement extends PDFElement {
	...
	render(layout, callback) {
		this.applyStyle(layout, this.area);

		let p = this.area.pixels;
		let pdf = layout.pdf;
		let options = { align: this.alignContent };
		let addTop = Layout.inchesToPixels(Layout.ifNaN(this.margin.top, 0));
		let addBottom = Layout.inchesToPixels(Layout.ifNaN(this.margin.bottom, 0));
