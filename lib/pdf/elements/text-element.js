...
class TextElement extends PDFElement {
	...
	render(style, pdf, callback) {
		...
		let options = {};
		let addTop = isNaN(this.margin.top) ? 0 : PDFElement.inchesToPixels(this.margin.top);
		let addBottom = isNaN(this.margin.bottom) ? 0 : PDFElement.inchesToPixels(this.margin.bottom);
