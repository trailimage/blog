...
class PDFElement {
	 ...
	 * @param {Number} height
	 */
	positionWithin(left, top, width, height) {
		if (!isNaN(this.bottom) && isNaN(this.top) && !isNaN(this.height)) {
			// position relative to bottom
			this.top = height - (this.height + this.bottom + Layout.ifNaN(this.margin.bottom, 0));
		}

		if (!isNaN(this.right) && isNaN(this.left) && !isNaN(this.width)) {
			// position relative to right
			this.left = width - (this.width + this.right + Layout.ifNaN(this.margin.right, 0));
		}
	}
