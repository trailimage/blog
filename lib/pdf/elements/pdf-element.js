...
const TI = require('../../');
const is = TI.is;
const Offset = TI.PDF.Element.Offset;
...
class PDFElement {
	...
	constructor(style) {
		 ...
		 */
		this.bottom = NaN;
		/** @type {TI.PDF.Element.Offset} */
		this.offset = new Offset();
		/**
		 * Element dimensions relative to PDF page
		 * @type {TI.PDF.Element.Offset}
		 */
		this.pageOffset = new Offset();
		/** @type {Number} */
		this.minWidth = NaN;
		/** @type {Number} */
		this.minHeight = NaN;
	...
	/** @param {Number} n */
	set width(n) { this.offset.width = n; }
	/** @return {Number} */
	get width() { return this.offset.width; }

	/** @param {Number} n */
	set height(n) { this.offset.height = n; }
	/** @return {Number} */
	get height() { return this.offset.height; }

	/** @param {Number} n */
	set left(n) { this.offset.left = n; }
	/** @return {Number} */
	get left() { return this.offset.left; }

	/** @param {Number} n */
	set top(n) { this.offset.top = n; }
	/** @return {Number} */
	get top() { return this.offset.top; }
	 ...
	 * @returns {Boolean}
	 */
	get laidOut() { return !this.offset.isEmpty; }
	 ...
	 * @param {TI.PDF.Element.Offset} area
	 */
	positionWithin(area) {
	...
	explicitLayout(layout, area) {
		layout.applyTo(this);
		if (this.pageOffset.isEmpty) { this.offset.copy(this.pageOffset);	}
		if (is.value(area)) { this.pageOffset.add(area); }
	}
	 ...
	 */
	render(layout, callback) { this.explicitLayout(layout, this.offset); }
