...
class ElementGroup extends PDFElement {
	 ...
	 * @param {TI.PDF.Element.Offset} [offset] Page area
	 */
	explicitLayout(layout, offset) {
		super.explicitLayout(layout, offset);

		let center = (this.alignContent === TI.PDF.Align.Center);

		for (let el of this.elements) {
			el.explicitLayout(layout, this.pageOffset);
		}

		for (let el of this.elements) {
			el.scale(this.pageOffset);
			el.positionWithin(this.pageOffset);
	...
	implicitLayout(area) {
		...
		for (let el of this.elements) { el.implicitLayout(this.offset); }
	}
	 ...
	 * @return {TI.PDF.Element.Text|TI.PDF.Element.Image|TI.PDF.Element.Group}
	 */
