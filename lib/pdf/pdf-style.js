...
class PDFStyle {
	...
	preparePDF(title, author) {
		let rule = this.rules.defaultPage;

		let options = {
			size: [Element.inchesToPixels(rule.width), Element.inchesToPixels(rule.height)],
			layout: (rule.width > rule.height) ? Page.Layout.Landscape : Page.Layout.Portrait,
			margins: {
				top: Element.inchesToPixels(rule.margin.top),
				bottom: Element.inchesToPixels(rule.margin.bottom),
				left: Element.inchesToPixels(rule.margin.left),
				right: Element.inchesToPixels(rule.margin.right)
			},
			info: {
				Title: title,
				Author: author
	 ...
	 * @param {PDFElement|Page|ElementGroup} el
	 */
	applyTo(el) {
