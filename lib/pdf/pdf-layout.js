...
class PDFLayout {
	 ...
	 * @param {ElementBase|TI.PDF.Page|TI.PDF.Element.Group|PDFPage} el
	 * @returns {PDFDocument}
	 */
	addPage(el) {
	 ...
	 * @param {ElementBase|TI.PDF.Page|TI.PDF.Element.Group} el
	 */
	applyTo(el) {
		...
		if (rules !== undefined) {
			for (let r in rules) {
				switch (r) {
					...
					case "align": el.align.horizontal = rules[r]; break;
					case "verticalAlign": el.align.vertical = rules[r]; break;
