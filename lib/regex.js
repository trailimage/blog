...
module.exports = {
	 ...
	 * @returns {RegExp}
	 */
	get artist() { return new RegExp('(' + config.style.artistNames.join('|') + ')', 'gi'); },
	queryString: /\?.+$/,
	fileExt: /\.\w{2,4}$/,
	get newLine() { return /(\r\n|\n|\r)/gm; },
	get trailingWhiteSpace() { return /[\r\n\s]*$/g; },
	get lineBreak() { return /\r*\n/gi; },
	get subtitle() { return new RegExp(config.style.subtitleSeparator + '\\s*', 'g'); },
	footnote: {
		/**
	...
	tag: {
		 ...
		 * @returns {RegExp}
		 */
		get linkToUrl() { return /<a href=["']([^"']+)['"][^>]*>\1<\/a>/gi; },
		/**
