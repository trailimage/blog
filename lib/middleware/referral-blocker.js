...
/**
 * @alias Blog.Middleware.referralBlocker.filter
 */
module.exports.filter = function(req, res, next) {
	...
	if (is.value(referer)) {
		isSpam(format.topDomain(referer), yes => {
			if (yes) {
				db.log.warnIcon(Blog.icon.target, 'Spam blocked %s referer', referer);
				res.status(Blog.httpStatus.notFound).end();
			} else {
				next();
			}
		});
...
function downloadBlackList(callback) {
	if (isDownloading) {
		...
		isDownloading = true;
		db.log.infoIcon(Blog.icon.cloudDownload, 'Downloading spam referral black list');
		...
		request(config.referralSpam.listUrl, (error, response, body) => {
			...
			} else if (response.statusCode != Blog.httpStatus.ok) {
				db.log.error('%s returned status %s', config.referralSpam.listUrl, response.statusCode);
			} else {
				// list of non-empty lines
				list = blackList = body.split('\n').filter(i => !is.empty(i));
				lastUpdate = new Date().getTime();
				db.log.infoIcon(Blog.icon.banned, 'Downloaded %d blocked domains', list.length);
