...
const CacheItem = require('../cache/cache-item.js');
...
module.exports.methods = (req, res, next) => {
	 ...
	 */
	res.notFound = () => {
		db.log.warnIcon(Enum.icon.x, `${req.originalUrl} not found for ${format.IPv6(req.connection.remoteAddress)}`);
...
function makeRenderer(res, slug, mimeType) {
	return (view, options, postProcess) => {
		...
		res.render(view, options, (renderError, text) => {
			if (renderError) {
				db.log.error('Rendering “%s”: %s', view, renderError.message);
