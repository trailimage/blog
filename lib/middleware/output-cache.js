...
const CacheItem = Blog.Cache.Item;
const is = Blog.is;
 ...
 * @alias Blog.Middleware.outputCache
...
module.exports.methods = (req, res, next) => {
	...
	res.sendView = (slug, p2, p3) => {
		let mimeType = (typeof p2 === 'string') ? p2 : Blog.mimeType.html;
	...
	res.sendJson = (slug, render) => {
		checkCache(res, slug, Blog.mimeType.json, render);
	};
...
function prepare(res, slug, mimeType, callback) {
	if (mimeType === Blog.mimeType.json) {
