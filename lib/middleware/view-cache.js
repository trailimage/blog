...
module.exports.apply = (req, res, next) => {
	...
	res.sendView = (slug, p2, p3) => {
		let mimeType = (typeof p2 === is.type.STRING) ? p2 : C.mimeType.HTML;
	...
	res.sendJson = (slug, render) => {
		checkCache(res, slug, C.mimeType.JSON, render);
	};
	 ...
	 * @returns {Promise}
	 */
	res.getCacheKeys = () => cache.keys(key);
	 ...
	 * @returns {Promise}
	 */
	res.removeFromCache = slugs => cache.remove(key, slugs);
