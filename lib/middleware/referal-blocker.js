...
module.exports.filter = function(req, res, next) {
	...
	if (is.value(referer)) {
		isSpam(format.topDomain(referer), yes => {
			if (yes) {
				db.log.warnIcon(Enum.icon.target, 'Spam blocked %s referer', referer);
 ...
 */
function isStale() {
	return lastUpdate === 0 || (new Date().getTime() - lastUpdate  > config.referalSpam.updateFrequency);
}
...
function downloadBlackList(callback) {
	if (isDownloading) {
		...
		isDownloading = true;
		db.log.infoIcon(Enum.icon.cloudDownload, 'Downloading spam referal black list');
		...
		request(config.referalSpam.listUrl, (error, response, body) => {
			...
			if (error !== null) {
				...
				db.log.infoIcon(Enum.icon.banned, 'Downloaded %d blocked domains', list.length);
