...
class FlickrFactory extends FactoryBase {
	constructor() {
		super();
		this.sizeField.coverPhoto = size.square150;
	...
	/**
	 * @param {TI.Library} library New or existing library
	...
	buildPostTag(collection) {
		...
		pt.icon = TI.PostTag.inferIcon(collection.title);
		pt.tags = [];
		pt.posts = [];
		return pt;
	}
	...
	buildSearchPhoto(s, sizeField) {
		...
		p.id = s.id;
		p.size.coverPhoto = this.buildPhotoSize(s, sizeField);
		return p;
	}
