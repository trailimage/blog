...
class FlickrPhoto extends PhotoBase {
	...
	loadPostPhotos(post, callback) {
		if (post.photosLoaded) {
			...
			let options = {
				extras: this.factory.sizesForPost.concat(['description', 'tags', 'date_taken', 'geo']).join()
			};
...
// - Tokens -------------------------------------------------------------------
	...
	getRequestToken(callback) {
		oauth.getOAuthRequestToken((error, token, secret) => {
			...
			this.options.oauth.requestToken = token;
			this.options.oauth.tokenSecret = secret;
	...
	getAccessToken(token, verifier, callback) {
		oauth.getOAuthAccessToken(token, this.options.oauth.tokenSecret, verifier, (error, accessToken, accessTokenSecret) => {
			this.options.oauth.tokenSecret = null;
			if (error) {
				log.error(error);
				callback(null, null);
			} else {
...
// - Private methods ----------------------------------------------------------
	...
	_signedCall(method, idType, id, callback, args) {
		...
		this.oauth.get(url, this.options.oauth.token, this.options.oauth.tokenSecret, (error, data) => {
			if (error) {
				log.error(error);
