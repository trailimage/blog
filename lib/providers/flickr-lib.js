...
const PhotoTag = require('../models/photo-tag.js');
...
class FlickrProvider extends LibraryProvider {
	...
	_loadPostsFromSource(callback, photoTags) {
		this._call('collections.getTree', 'user_id', this.options.userID, r => {
			if (is.value(r) && r.stat == 'ok') {
				...
				setTimeout(() => { this._loadPostsFromSource(callback, photoTags); }, this.options.retryDelay);
			}
		});
	}
	...
	_loadPhotoTagsFromSource(callback) {
		this._signedCall('tags.getListUserRaw', null, null, r => {
			if (is.value(r) && r.stat == 'ok') {
				...
				callback(PhotoTag.parse(tags, this.options.excludeTags));
			} else {
	...
	_parsePostPhotos(post, setPhotos) {
		...
		post.photos = setPhotos.photo.map((p, index) => Photo.parse(p, this.sizeField, index));
	...
	photosWithTags(tags, callback) {
		...
		this._signedCall('photos.search', 'user_id', this.options.userID, r => {
			callback(is.value(r) ? r.photos.photo.map(p => Photo.parseThumb(p, this.sizeField.thumb)) : null);
		},
		options);
	}
