...
const OAuthBase = app.Auth.Base;
const Point = app.Map.Point;
const FeatureCollection = app.Map.FeatureList;
...
class FileBase extends OAuthBase {
	...
	_makeMap(slug, callback) {
		...
		let post = app.Library.postWithSlug(slug);
	...
	_saveMap(geo, post, callback) {
		...
		compress.gzip(JSON.stringify(geo), (err, buffer) => {
			db.log.infoIcon(Enum.icon.globe, 'Loaded and compressed GeoJSON for "%s"', post.title);
