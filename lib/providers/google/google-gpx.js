 ...
 */
class GoogleGPX extends MapBase {
	constructor(options) {
		super();
	...
	loadGPX(post, callback) {
		...
		this.drive.children.list(options, (err, list) => {
			if (err != null) {
				...
				log.info('%s GPX for "%s" (%s)', purpose, post.title, item.id);
				this._downloadFile(item.childLink, slug, callback);
			}
		});
	}
	 ...
	 * @private
	 */
	_downloadFile(url, slug, callback) {
		url = url + '?alt=media';
		let options = {
			url: url,
			headers: {
		...
		if (callback instanceof ServerResponse) {
			...
			request(options, (err, response, body) => {
				if (err == null) {
					log.info('Completed GPX retrieval from %s', url);
					callback(body);
				} else {
