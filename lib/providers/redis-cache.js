...
class RedisProvider extends CacheProvider {
	...
	constructor(host) {
		...
		this.remote = config.fromUrl(host);

		/** @type {RedisClient} */
		this.client = Redis.createClient(this.remote.port, this.remote.host, {
			max_attempts: 2
		});
		...
		this.client.on('error', err => {
			if (err.code == code.BROKEN) {
				...
				log.error("Unable to connect to Redis at %s:%d", this.remote.host, this.remote.port);
				this.ready = false;
	...
	authorize() {
		this.client.auth(this.remote.password);
	}
