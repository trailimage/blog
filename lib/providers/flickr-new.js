...
const PhotoProvider = require('./cache-provider');
...
class FlickrProvider extends PhotoProvider {
	...
	exif(photoID, callback) {
		call('photos.getExif', 'photo_id', photoID, function(r) {
			callback((r) ? EXIF.parse(r.photo.exif) : null);
		});
	}

	library(callback) {
		call('collections.getTree', 'user_id', setting.flickr.userID, r => {
			callback((r) ? r.collections : null);
		});
	}
	 ...
	 * @param {String|Post} postOrID Flickr set ID
	...
	post(postOrID, callback) {
		...
		call('photosets.getPhotos', 'photoset_id', id, r => {
			...
			if (alsoGetInfo) {
				...
				exports.getSetInfo(id, info => { callback(r.photoset, info); });
			} else {
				callback(r.photoset, null);
			}
		}, options);
	}
	...
	photosWithTags(tags, callback) {
		...
		if (!is.array(sizes) || sizes.length == 0) { sizes = [exports.size.thumbnail]; }

		signedCall(service, 'user_id', setting.flickr.userID, r => { callback(is.value(r) ? r.photos.photo : null); }, options);
	}
