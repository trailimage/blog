...
const MemoryProvider = require('../providers/memory-cache.js');
...
class CacheHelper {
	...
	constructor(db) {
		...
		if (!(db instanceof MemoryProvider)) {
			...
			db.once(db.eventType.CONNECTED, () => { this.pendingAction = []; });
			db.once(db.eventType.BROKEN, () => {
				...
				for (let command of this.pendingAction) {
	...
	add(key, p2, p3, p4) {
		...
		if (!db.connected) { this.pendingAction.push({ add: arguments }); }
	...
	addAll(key, hash, callback) {
		...
		if (!db.connected) { this.pendingAction.push({ addAll: arguments }); }
