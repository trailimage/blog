...
class FlickrProvider extends LibraryProvider {
	...
	_loadFromSource(callback) {
		call('collections.getTree', 'user_id', config.flickr.userID, r => {
			if (is.value(r)) {
				...
				log.warn('Retrying in %d seconds', (config.retryDelay / Enum.time.second));
				setTimeout(() => { this._loadFromSource(callback); }, config.retryDelay);
			}
		});
	}
	...
	_parsePostPhotos(post, setPhotos) {
		...
		if (post.id != config.flickr.photoSet.poetry && post.id != config.flickr.photoSet.featured) {
...
function retry(fn, args) {
	...
	if (count > config.flickr.maxRetries) {
		retries[key] = 0;
		log.error('Call to %s with %s failed after %s tries', method, id, config.flickr.maxRetries);
		callback(null);
	} else {
		log.warn('Retry %s for %s with %s', count, method, id);
		setTimeout(() => { fn.apply(scope, args); }, config.flickr.retryDelay);
	}
}
...
function signedCall(method, idType, id, callback, args) {
	...
	oauth.get(url, config.flickr.token, config.flickr.tokenSecret, (error, data) => {
		if (error) {
			log.error(error);
...
function parameterize(method, idType, id, args) {
	...
	args.api_key = config.flickr.key;
	args.format = 'json';
