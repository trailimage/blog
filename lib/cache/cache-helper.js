...
class CacheHelper {
	...
	keys(key, callback) {
		...
		if (!db.connected) {
			log.warn('Attempted to get "%s" keys from disconnected cache', key);
			callback(null);
		} else {
			if (/[\?\*\[\]]/.test(key)) {
	...
	_getValue(type, key, p2, p3) {
		...
		if (!db.connected) {
			log.warn('Attempted to get "%s" from disconnected cache', key);
			callback(null);
		} else {
			if (p3 === undefined) {
				db.select(key, db.responder(key, callback, type));   // http://redis.io/commands/get
			} else {
				db.selectMember(key, p2, db.responder(key, callback, type));
			}
		}
	}
	...
	addOutput(key, slug, buffer, callback) {
		this.add(key, slug, new CacheItem(slug, buffer), callback);
	}
}
