...
class CacheHelper {
	...
	remove(key, p2, p3) {
		...
		if (is.empty(key)) {
			...
			if ((is.array(p2) && p2.length === 0) || is.empty(p2)) {
				...
				db.removeMember(key, p2, db.responder(p2, callback, CacheBase.replyType.NONE));
			}
		} else {
			db.remove(key, db.responder(key, callback, CacheBase.replyType.COUNT));
		}
	}
	...
	exists(key, p2, p3) {
		...
		if (p3 === undefined) {
			// p2 is the callback
			db.exists(key, db.responder(key, p2, CacheBase.replyType.BIT));
		} else {
			// p2 is a field name, p3 is the callback
			db.memberExists(key, p2, db.responder(key, p3, CacheBase.replyType.BIT));
		}
	}
	...
	keys(key, callback) {
		...
		if (/[\?\*\[\]]/.test(key)) {
			...
			db.keys(key, db.responder(key, callback, CacheBase.replyType.RAW));
		} else {
			// all fields of a hash key
			db.memberKeys(key, db.responder(key, callback, CacheBase.replyType.RAW));
		}
	}
	 ...
	 */
	get(key, p2, p3) { this._getValue(CacheBase.replyType.RAW, key, p2, p3); }
	 ...
	 * @param {function(Object)} [p3] Callback if hash key given
	 */
	getObject(key, p2, p3) { this._getValue(CacheBase.replyType.JSON, key, p2, p3); }
	...
	getAll(key, callback) {
		let db = this.provider;
		db.selectAll(key, db.responder(key, callback, CacheBase.replyType.RAW));
	};
	...
	add(key, p2, p3, p4) {
		...
		if (p4 !== undefined || (p3 !== undefined && !(p3 instanceof Function))) {
			db.addMember(key, p2, db.normalize(p3), db.responder(key, p4, CacheBase.replyType.NONE));
		} else if (p3 !== undefined) {
			db.add(key, db.normalize(p2), db.responder(key, p3, CacheBase.replyType.OKAY));
	...
	addAll(key, hash, callback) {
		...
		db.addAll(key, hash, db.responder(key, callback, CacheBase.replyType.OKAY));
	};
}
