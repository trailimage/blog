...
class CacheHelper {
	...
	constructor(p) {
		...
		if (!(p instanceof MemoryCache)) {
			...
			p.once(CacheBase.EventType.CONNECTED, () => { this._pendingAction = []; });
			p.once(CacheBase.EventType.FATAL, () => {
	...
	remove(key, p2, p3) {
		...
		if (is.empty(key)) {
			...
			if ((is.array(p2) && p2.length === 0) || is.empty(p2)) {
				...
				this.provider.removeMember(key, p2, this.provider.responder(p2, callback, CacheBase.DataType.NONE));
			}
		} else {
			this.provider.remove(key, this.provider.responder(key, callback, CacheBase.DataType.COUNT));
		}
	}
	...
	keys(key, callback) {
		...
		if (!p.connected) {
			...
			if (/[\?\*\[\]]/.test(key)) {
				...
				p.keys(key, p.responder(key, callback, CacheBase.DataType.RAW));
			} else {
				// all fields of a hash key
				p.memberKeys(key, p.responder(key, callback, CacheBase.DataType.RAW));
			}
		}
	}
	...
	getAll(key, callback) {
		...
		if (!p.connected) {
			...
			p.selectAll(key, p.responder(key, callback, CacheBase.DataType.RAW));
		}
	};
	...
	add(key, p2, p3, p4) {
		...
		if (p4 !== undefined || (p3 !== undefined && !(p3 instanceof Function))) {
			p.addMember(key, p2, p.normalize(p3), p.responder(key, p4, CacheBase.DataType.NONE));
		} else if (p3 !== undefined) {
			p.add(key, p.normalize(p2), p.responder(key, p3, CacheBase.DataType.OKAY));
	...
	addAll(key, hash, callback) {
		...
		p.addAll(key, hash, p.responder(key, callback, CacheBase.DataType.OKAY));
	};
