 ...
 * @alias TI.Provider.Cache.Helper
 */
class CacheHelper {
	/**
	 * @param {Blog.Provider.Cache.Base|EventEmitter} p
	 */
	constructor(p) {
		this.provider = p;
		/**
		...
		if (!(p instanceof MemoryCache)) {
			...
			p.once(Cache.EventType.CONNECTED, () => { this._pendingAction = []; });
			p.once(Cache.EventType.FATAL, () => {
				TI.log.error("Failing over to in-memory hash");
	...
	remove(key, p2, p3) {
		...
		if (is.empty(key)) {
			TI.log.error('Attempt to delete hash item with empty key');
			callback(false);
		} else if (!p.connected) {
			TI.log.warn('Attempt to delete "%s" field from disconnected cache', key);
			callback(false);
		} else if (is.value(p3) || !(is.callable(p2))) {
			// implies that hash field is the second argument
			if ((is.array(p2) && p2.length === 0) || is.empty(p2)) {
				TI.log.error('Attempt to delete "%s" field with empty field name', key);
				callback(false);
			} else {
				...
				p.removeMember(key, p2, p.responder(p2, callback, Cache.DataType.NONE));
			}
		} else {
			p.remove(key, p.responder(key, callback, Cache.DataType.COUNT));
		}
	}
	...
	exists(key, p2, p3) {
		...
		if (p3 === undefined) {
			// p2 is the callback
			p.exists(key, p.responder(key, p2, Cache.DataType.BIT));
		} else {
			// p2 is a field name, p3 is the callback
			p.memberExists(key, p2, p.responder(key, p3, Cache.DataType.BIT));
		}
	}
	...
	keys(key, callback) {
		...
		if (!p.connected) {
			TI.log.warn('Attempted to get "%s" keys from disconnected cache', key);
			callback(null);
		} else {
			if (/[\?\*\[\]]/.test(key)) {
				// pattern match against root keys
				p.keys(key, p.responder(key, callback, Cache.DataType.RAW));
			} else {
				// all fields of a hash key
				p.memberKeys(key, p.responder(key, callback, Cache.DataType.RAW));
			}
		}
	}
	 ...
	 */
	get(key, p2, p3) { this._getValue(Cache.DataType.RAW, key, p2, p3); }
	 ...
	 */
	getObject(key, p2, p3) { this._getValue(Cache.DataType.JSON, key, p2, p3); }
	...
	_getValue(type, key, p2, p3) {
		...
		if (!p.connected) {
			TI.log.warn('Attempted to get "%s" from disconnected cache', key);
			callback(null);
		} else {
			if (p3 === undefined) {
	...
	getAll(key, callback) {
		...
		if (!p.connected) {
			...
			p.selectAll(key, p.responder(key, callback, Cache.DataType.RAW));
		}
	};
	...
	add(key, p2, p3, p4) {
		...
		if (p4 !== undefined || (p3 !== undefined && !(p3 instanceof Function))) {
			p.addMember(key, p2, p.normalize(p3), p.responder(key, p4, Cache.DataType.NONE));
		} else if (p3 !== undefined) {
			p.add(key, p.normalize(p2), p.responder(key, p3, Cache.DataType.OKAY));
	...
	addAll(key, hash, callback) {
		...
		p.addAll(key, hash, p.responder(key, callback, Cache.DataType.OKAY));
	};
