	...
	responder(key, callback, type) {
		return (err, reply) => {
			...
			if (is.callable(callback)) {
				let response = null;
				if (type === undefined) { type = CacheBase.replyType.NONE; }

				if (error) {
					if (type !== CacheBase.replyType.RAW && type !== CacheBase.replyType.JSON) { response = false; }
				} else {
					switch (type) {
						case CacheBase.replyType.BIT: response = this.isTrue(reply); break;
						case CacheBase.replyType.OKAY: response = this.isOkay(reply); break;
						case CacheBase.replyType.COUNT: response = (reply == howMany(key)); break;
						case CacheBase.replyType.RAW: response = reply; break;
						case CacheBase.replyType.JSON: response = this.parseObject(reply); break;
						case CacheBase.replyType.NONE: response = true; break;
					}
				}
				callback(response);
			}
