 ...
 * Allow wildcard rules to match Redis functionality
 * @param {String} key
 * @param {String} [memberKey]
 * @returns {Boolean}
 */
function contains(key, memberKey) {
	let reKey = ruleToPattern(key);
	let keys = Object.keys(db).filter(k => reKey.test(k));
	let found = keys.length > 0;

	if (found && memberKey !== undefined) {
		let reField = ruleToPattern(memberKey);
		found = false;

		for (k of keys) {
			let index = Object.keys(db[k]).findIndex(f => reField.test(f));
 ...
 * @param {String} pattern
 */
function containsKey(hash, pattern) {
	let re = ruleToPattern(key);
	let keys = Object.keys(db).filter(k => re.test(k));
}
