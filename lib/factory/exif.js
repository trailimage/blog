...
function sanitizeExif(exif) {
    ...
    const camera = (text) => is_1.default.empty(text) ? '' : text
        .replace('NIKON', 'Nikon')
    ...
    const lens = (text, camera) => is_1.default.empty(text) ? '' : text
    ...
    const software = (text) => is_1.default.empty(text) ? '' : text
    ...
    if (!exif.sanitized) {
        if (is_1.default.value(exif.artist) && regex_1.default.artist.test(exif.artist)) {
            ...
            if (!numericRange.test(exif.lens)) {
