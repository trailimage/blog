...
function buildLibrary(emptyIfLoaded = true) {
    ...
    return Promise.all([flickr.getCollections(), flickr.getAllPhotoTags()])
        ...
        Promise.all(library_1.default.posts.map(p => p.getInfo())).then(() => {
...
function getPostWithPhoto(photo) {
    const id = typeof photo == is_1.default.type.STRING ? photo : photo.id;
    return flickr
        .getPhotoContext(id)
        .then(sets => is_1.default.value(sets) ? this.posts.find(p => p.id == sets[0].id) : null);
}
function getEXIF(photoID) {
    return flickr.getExif(photoID).then(exif_1.default.make);
}
const getPhotosWithTags = (tags) => flickr.photoSearch(tags).then(photos => photos.map(json => ({
    id: json.id,
    size: {
        thumb: photo_size_1.default.make(json, config_1.default.flickr.photoSize.search[0])
    }
})));
function parsePhotoTags(rawTags) {
    const exclusions = is_1.default.array(config_1.default.flickr.excludeTags)
        ? config_1.default.flickr.excludeTags
        : [];
