...
function hasKey(key) {
    return (this.key == key ||
        (is_1.default.value(this.partKey) && key == this.seriesKey + '-' + this.partKey));
}
...
function getPhotos() {
    return this.photosLoaded
        ...
        : flickr
            ...
            .then((res) => updatePhotos(this, res));
}
function getInfo() {
    return this.infoLoaded
        ...
        : flickr
            .getSetInfo(this.id)
            .then((info) => updateInfo(this, info));
}
...
function updatePhotos(p, setPhotos) {
    ...
    if (p.photos.length > 0) {
        ...
        if (!is_1.default.empty(p.description)) {
            ...
            p.longDescription +=
                is_1.default.value(p.video) && !p.video.empty ? ' and one video)' : ')';
        }
...
function name() {
    ...
    return (p.title +
        (p.isPartial ? config_1.default.library.subtitleSeparator + ' ' + p.subTitle : ''));
}
