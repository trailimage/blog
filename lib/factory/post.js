...
function updatePhotos(p, setPhotos) {
    ...
    if (p.photos.length > 0) {
        ...
        if (!is_1.default.value(p.coverPhoto)) {
            logger_1.default.error('No cover photo defined for %s', this.title);
...
function updatePhotoMarkers() {
    ...
    for (let i = start; i < total; i++) {
        ...
        if (img.latitude > 0) {
            map += '|' + img.latitude + ',' + img.longitude;
        }
    }
    this.photoMarkers = (is_1.default.empty(map)) ? null : encodeURIComponent('size:tiny' + map);
}
