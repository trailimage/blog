...
function mapJSON(req, res) {
   const key = req.params[ph.POST_KEY];
   cache.map.getItem(key)
      .then(item => {
         if (is.value(item)) {
            return item;
         } else {
            const post = library.postWithKey(key);
            if (!is.value(post)) { throw new ReferenceError(`Post ${key} could not be loaded`); }

            google.drive.loadGPX(post).then(map.featuresFromGPX).then(geo => {
               cache.map.add(key, geo);
            });
         }
      })
      .catch(err => {
