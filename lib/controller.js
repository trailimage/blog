...
module.exports = {
   ...
   siteMap(req, res) {
      res.sendView(template.page.sitemap, e.mimeType.XML, render => {
   ...
   issue(req, res) {
      res.redirect(e.httpStatus.PERMANENT_REDIRECT, 'http://issues.' + config.domain);
   },
   ...
   rss(req, res) {
      ...
      if (!library.postInfoLoaded) {
         if (retries >= MAX_RETRIES) {
            ...
            res.render(e.httpStatus.NOT_FOUND, {'title': 'Unable to load feed'});
      ...
      res.set('Content-Type', e.mimeType.XML);
