...
function mapGPX(req, res) {
   ...
   if (is.value(post)) {
      res.setHeader(C.header.content.DISPOSITION, `attachment; filename=${post.key}.gpx`);
      res.setHeader(C.header.content.TYPE, C.mimeType.GPX);
      google.drive.loadGPX(post, res).then(()=> res.end());
   } else {
      res.notFound();
   }
}
...
function photosWithTag(req, res) {
   ...
   library.getPhotosWithTags(slug).then(photos => {
      if (photos === null || photos.length == 0) {
         ...
         let title = util.number.say(photos.length) + ' &ldquo;' + tag + '&rdquo; Image' + ((photos.length != 1) ? 's' : '');
...
function deleteViewCache(req, res) {
   ...
   let apiHashKeys = [];
   let removals = [];
   let includeRelated = req.body['includeRelated'] == 'true';
