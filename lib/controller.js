...
function mapJSON(req, res) {
   db.file.loadMap(req.params[ph.POST_KEY], item => {
      if (item != null) {
...
/**
 * Update library from photo provider, usually to find new posts
...
function updateLibrary() {
   return library.load(false).then(changedKeys => {
      if (changedKeys.length > 0) {
         ...
         return res.removeFromCache(changedKeys).then(() => Promise.resolve(changedKeys));
      } else {
...
/**
 * Delete view cache item and those related to it
 */
function deleteViewCache(req, res) {
   // cache keys to be invalidated
   let keys = menuKeys.slice();
   ...
   for (let key of req.body.selected) {
      ...
      if (p !== null) {
         ...
         keys = keys.concat(Object.keys(p.categories));
         // and adjacent views
         if (p.next !== null) { keys.push(p.next.key); }
         if (p.previous !== null) { keys.push(p.previous.key); }
      }
   }

   Promise.all([
      // remove Flickr responses
      // TODO must target getInfo and getPhotos and maybe member photos
      cache.remove(keys),
      // remove rendered and compressed views
      cache.view.remove(keys)
   ])
   .then(() => {
