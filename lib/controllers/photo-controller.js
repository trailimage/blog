...
exports.exif = (req, res) => {
    ...
    flickr.getEXIF(photoID, exif => {
       ...
       res.render(template.page.exif, { exif: values, layout: template.layout.none });
    });
};
...
exports.view = (req, res) => {
	...
	flickr.getContext(photoID, sets => {
		if (sets !== null) {
			for (let s of sets) {
				// photo could be in more than one set — show whichever is first, if any
				let postID = s.id;
...
exports.search = (req, res) => {
	...
	flickr.tagSearch([slug], Photo.sizesForSearch, photos => {
		if (photos === null) {
			res.notFound();
		} else {
			let matches = photos.map(p => Photo.parseThumb(p));
			let tag = library.photoTags[slug];
			let title = format.sayNumber(matches.length) + ' &ldquo;' + tag + '&rdquo; Image' + ((matches.length != 1) ? 's' : '');
