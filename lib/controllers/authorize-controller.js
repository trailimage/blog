...
exports.flickr = (req, res) => {
	...
	if (flickr instanceof FlickrProvider.Photo) {
		if (is.empty(req.param('oauth_token'))) {
			db.log.warn('%s is updating Flickr tokens', req.clientIP());
...
exports.google = (req, res) => {
	let provider = Blog.active.file;

	if (provider instanceof GoogleProvider.File) {
