...
function photoJSON(_req, res) {
    _1.default.map
        .photos()
        .then(item => {
        res.sendCompressed(constants_1.mimeType.JSON, item);
    })
        .catch(err => {
...
function trackJSON(req, res) {
    _1.default.map
        .track(req.params[constants_1.route.POST_KEY])
        .then(item => {
        res.sendCompressed(constants_1.mimeType.JSON, item);
    })
        .catch(err => {
...
const fetchKMZ = (sourceName) => (res) => res
    .buffer()
    .then(kml_1.default.fromKMZ)
    .then(geojson_1.default.featuresFromKML(sourceName));
function gpx(req, res) {
    const post = config_1.default.map.allowDownload
        ? library_1.default.postWithKey(req.params[constants_1.route.POST_KEY])
        : null;
    if (is_1.default.value(post)) {
        google.drive
            .loadGPX(post, res)
