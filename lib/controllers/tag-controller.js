...
exports.root = (req, res) => {
	...
	if (is.value(slug)) {
		res.sendView(slug, render => {
			...
			let tag = library.categoryWithSlug(slug);
			...
			if (is.value(tag)) {
				...
				renderView(render, template.page.POST_TAG_CATEGORY, tag, ld, options, count, config.site.postTagAlias);
			} else {
				res.NOT_FOUND();
			}
		});
...
exports.menu = (req, res) => {
	const t = template.page.TAG_MENU;
	...
	res.sendView(slug, render => {
		...
		if (is.value(tag)) {
			tag.ensureLoaded(TI.active.photo, ()=> {
				...
				renderView(render, template.page.POST_TAG, tag, ld, options, count, subtitle);
			});
		} else {
			res.NOT_FOUND();
		}
