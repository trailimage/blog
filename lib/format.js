...
exports.tagList = list => {
	...
	if (list) {
		...
		for (let t of tags) {
			if (setting.removeTag.includes(t)) {
...
exports.text = text =>
	is.empty(text) ? null : text
		...
		.replace(/(<a [^>]+>)([^<]+)<\/a>/gi, (match, tag, url) => tag.replace(/(&ldquo;|&rdquo;)/g, '"') + url + '</a>');
...
exports.paragraphs = text => {
	if (!exports.isEmpty(text))	{
		...
		text = text.replace(Enum.pattern.poetry, (match, c1, c2) => {
		...
		text = text.replace(Enum.pattern.blockQuote, (match, c1) => '[Q]' + c1.replace(/[“”]/g, '') + '[/Q]\n\r');
		text = '<p>' + this.text(text) + '</p>';

		text = text
			.replace(Enum.pattern.link, (match, url) => {
...
exports.fixFlickr = text => {
	...
	text = text.replace(Enum.pattern.badLinkTag, (match, part, i) => {
		index = i;
		return part + '</a>';
	});
	...
	if (index > 0) {
		...
		let newLink = oldLink.replace(Enum.pattern.link, (match, p1, p2) => {
 ...
 * @static
 */
exports.isEmpty = text => is.empty(text);