 ...
 */
exports.IPv6 = ip => (ip === '::1') ? '127.0.0.1' : ip.remove(/^::[0123456789abcdef]{4}:/g);
	...
	if (!is.empty(text)) {
		if (re.poetry.all.test(text)) {
			...
			if (re.haiku.all.test(text)) {
				...
				// not hiaku
				text = '<p class="poem">'
					+ text.replace(re.lineBreak, '<br/>').replace(re.poetry.indent, '<span class="tab"></span>')
					+ '</p>';
			}
...
function formatCaption(text) {
	if (!is.empty(text))	{
		...
		text = fixFlickrLinks(text)
			...
			.replace(re.quote.block, (match, body) => '[Q]' + body.remove(re.quote.curly) + '[/Q]\n\r');

		text = '<p>' + exports.typography(text) + '</p>';

		text = text
			.replace(re.tag.link, shortenLinkText)
			.replace(re.newLine, '</p><p>')
			.replace(re.quip, (match, tag, body) => '<p class="quip">' + body)
			.replace(re.footnote.text, formatNotes)
 ...
 * @param {String} url URL capture
 * @returns {string}
 */
function shortenLinkText(match, url) {
 ...
 * @param {String} match RegEx match
 * @param {String} underscores Leading underscore capture
 * @param {String} notes Text capture
 * @return {string}
 */
function formatNotes(match, underscores, notes) {
	for (let i = 1; i < 10; i++) {
		let re = new RegExp('[' + superscript[i] + ']\s*', 'g');
		notes = notes.replace(re, `<sup>${i}.</sup>`);
	}
	return '<div class="footnotes">'
		+ notes.replace(/[\*]\s*/g, '<sup class="title">*</sup>')
		+ exports.icon('book') + '</div>';
}
...
function fixFlickrLinks(text) {
	...
	if (index > 0) {
		...
		let newLink = oldLink.replace(re.tag.link, (match, url, name) => {
			...
			return exports.string('<a href="{0}">{1}</a>', name, url.remove(protocol));
		});
...
function formatHaiku(text, regex) {
	...
	return '<p class="haiku">'
		...
		+ formatCaption(text.remove(match[0]));
}
...
function formatPoem(text) {
	...
	if (re.quote.open.test(p) && re.quote.end.test(p)) {
		// make an HTML blockquote out of poem surrounded by quotes
		p = p.remove(re.quote.any);
		tag = 'blockquote';
	}
	p = p.replace(re.footnote.number, '$1<sup>$2</sup>');
