...
module.exports = {
   ...
   quip: /(<p>)(“(?=[^<]*”)[^<]{4,80}<\/p>)/i,
   get artist() { return new RegExp('(' + config.library.artistNames.join('|') + ')', 'gi'); },
   queryString: /\?.+$/,
   fileExt: /\.\w{2,4}$/,
   get newLine() { return /(\r\n|\n|\r)/gm; },
   get trailingWhiteSpace() { return /[\r\n\s]*$/g; },
   get lineBreak() { return /\r*\n/gi; },
   get subtitle() { return new RegExp(config.library.subtitleSeparator + '\\s*', 'g'); },
   footnote: {
