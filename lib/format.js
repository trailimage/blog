...
exports.capitalize = function(text) {
	return (is.empty(text)) ? '' : text.substr(0,1).toUpperCase() + text.substr(1).toLowerCase();
};
...
exports.parseTimeStamp = function(timestamp) {
	return (is.date(timestamp)) ? timestamp : new Date(timestamp * 1000);
};
...
exports.slug = function(text) {
	return (is.empty(text)) ? null : text.toLowerCase().replace(/\s/g, '-').replace(/[^\-a-z0-9]/g, '');
};
...
exports.text = function(text) {
	return (is.empty(text)) ? null : text
		.replace(/(\w)'/g, '$1&rsquo;')
 ...
 * @static
 */
exports.isEmpty = function(text) { return is.empty(text); };