...
module.exports = {
   ...
   date: d => Enum.month[d.getMonth()] + ' ' + d.getDate() + ', ' + d.getFullYear(),
   ...
   // stylize punctuation
   typography: text => is.empty(text) ? '' : text
   ...
   shortenLinkText: text => text.replace(re.tag.linkToUrl, (match, protocol, url) => {
      let parts = url.split('/');
   ...
   postTagIcon(title) {
      ...
      if (is.value(map)) {
         for (let name in map) { if (name == title) { return exports.icon(map[name]); } }
         if (is.defined(map,'default')) { return exports.icon(map['default']); }
      }
      return '';
   },
...
function formatHaiku(text, regex) {
   ...
   return '<p class="haiku">'
      + match[1] + '<br/>'
      + match[2] + '<br/>'
      + match[3] + exports.icon(config.style.icon.haiku) + '</p>'
