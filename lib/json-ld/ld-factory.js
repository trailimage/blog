...
class LinkDataFactory {
	...
	static fromPost(post) {
		...
		ld.articleSection = post.tags.join(',');
	...
	static fromPostTag(tag, slug, homePage) {
		...
		if (homePage){
			...
			ld = new Blog();
			ld.mainEntityOfPage = LinkDataFactory.webPage(slug);
			ld.name = tag.title;
			ld.publisher = LinkDataFactory.organization();
		}
		return ld;
	 ...
	 */
	static siteLogo() {
		return Image.fromURL(config.site.logoURL, 96, 96);
	}
	 ...
	 */
	static ownerImage() {
		return Image.fromURL(config.owner.image.url, config.owner.image.width, config.owner.image.height);
	}
