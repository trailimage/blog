...
const PhotoTag = require('./photo-tag.js');
...
class Library {
	...
	_addFlickrCollection(collection, root) {
		...
		if (root) {
			...
			this._addPost(Post.parse({'id': setting.flickr.set.poetry, 'title': 'Ruminations'}, false));
		}
		return t;
	}
	...
	_addPost(p) {
		...
		if (p.chronological && this.posts.length > 1) {
			/** @type {Post} */
			let next = exports.posts[this.posts.length - 2];
