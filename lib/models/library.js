...
class Library {
		...
		if (is.array(names) && names.length > 0) {
			...
			for (let childName of names) {
				for (let key in this.tags) {
					...
					if (is.value(childTag)) { slugs.push(childTag.fullSlug); }
				}
			}
		} else {
			...
			for (let key in this.tags) {
				...
				slugs.push(parentTag.slug);
				for (let childTag of parentTag.tags) { slugs.push(childTag.fullSlug); }
			}
		}
		return slugs;
	};
	 ...
	 * @returns {TI.PostTag}
	 */
	tagWithSlug(rootSlug, slug) {
		if (rootSlug.includes('/')) {
			let parts = rootSlug.split('/');
			rootSlug = parts[0];
			slug = parts[1];
		}
		...
		for (let key in this.tags) {
			...
			if (tag.slug == rootSlug) {
				return (slug === undefined) ? tag : tag.child(slug);
			}
		}
		return null;
	}
