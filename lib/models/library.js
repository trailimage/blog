...
class Library {
	...
	static load(callback) {
		if (instance !== null) {
			...
			config.provider.library.loadPosts(library => {
				instance = library;
				callback(library);
			});
		}
	 ...
	 * @return {Library}
	 */
	static parse(flickrTree, featureSets) {
		let library = new Library();
		for (let c of flickrTree.collection) { library._addFlickrCollection(c, true, featureSets); }
...
// - Private members ----------------------------------------------------------
	 ...
	 * @private
	 */
	_addFlickrCollection(collection, root, featureSets) {
		...
		if (root && is.array(featureSets)) {
			...
			for (let f of featureSets) {
				this._addPost(Post.parse(f, false));
			}
		}
		return t;
	}
