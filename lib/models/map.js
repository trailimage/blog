...
function addPhotos(geo, post, callback)
{
	post.getPhotos(function()
	{
		...
		for (var i = 0; i < post.photos.length; i++)
		{
			...
			{
				geo.features.push(parsePhoto(post.photos[i]));
			}
		}

		compress.gzip(JSON.stringify(geo), function(err, buffer)
		{
			db.add(key, post.slug,
			{
				'buffer': buffer.toString('hex'),
				'eTag': post.slug + '_map_' + (new Date()).getTime().toString()
			}, callback);
		});
	});
}
...
function parseGPX(gpx)
{
	...
	{
		geo.features.push(parseTrack(tracks[i]));
	}
	for (i = 0; i < routes.length; i++)
	{
		geo.features.push(parseRoute(routes[i]));
	}
	for (i = 0; i < waypoints.length; i++)
	{
		geo.features.push(parsePoint(waypoints[i]));
	}
	return geo;
}
...
function parseTrack(node)
{
	...
	stat.duration = parseFloat(((last[last.length - 1][point.time] - track[0][0][point.time]) / Enum.time.hour).toFixed(2));
...
function parseLine(node, name)
{
	...
	for (var i = 0; i < points.length; i++)
	{
		line.push(parseLocation(points[i]));
	}
	...
	for (i = 1; i < line.length; i++)
	{
		...
		d = length(line[i], line[i - 1]);                       // miles
		line[i][point.speed] = d/(t/Enum.time.hour);            // miles per hour
	}
...
function simplifyTrack(points)
{
	var len = points.length,
		...
		// equater = 131477280 feet
		// 365214 feet per degree
		// do deviation feet 5 = .0000136906
