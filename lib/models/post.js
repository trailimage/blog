 ...
 * @returns {boolean}
 */
Post.prototype.hasTags = () => this.tags.length > 0;
 ...
 */
Post.prototype.getPhotos = callback => {
	if (this.photosLoaded && this.infoLoaded) {
		...
		flickr.getSet(p.id, sizes, !p.infoLoaded, (photos, info) => {
			if (photos != null) {
...
Post.prototype.makeSeriesStart = () => {
 ...
 * @returns {boolean}
 */
Post.prototype.isMatch = slug =>
	(this.slug == slug || (this.partSlug != null && slug == this.seriesSlug + '-' + this.partSlug));

/**
 * Groups the items belong to are treated as tags or keywords
 * @param {String} tag
 */
Post.prototype.addTag = tag => {
 ...
 * @return {Post}
 */
exports.fromFlickr = (api, chronological) => {
	var p = new Post();
