	...
	addPhotos(list) {
		...
		if (this.photos.length > 0) {
			...
			if (is.value(firstDatedPhoto)) {
				this.dateTaken = format.date(firstDatedPhoto.dateTaken)
			}

			if (!is.empty(this.description)) {
				this.description = `${this.description} (Includes ${this.photos.length} photos`;
				this.description += (is.value(this.video) && !this.video.empty) ? ' and one video)' : ')';
			}
