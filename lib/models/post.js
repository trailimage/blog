...
class Post {
	...
	addPhotos(list) {
		...
		if (this.photos.length > 0) {
			...
			if (!is.value(this.coverPhoto)) {
				db.log.error('No cover photo defined for %s', this.title);
	...
	/**
	 * Reference a light-weight copy of the PostTag
	 * @alias TI.Post.addTag
	 * @param {TI.PostTag|PostTag} t
	 */
	addTag(t) { this.tags[t.slug] = t.title; };
	...
	_serializePhotoCoordinates() {
		...
		if (total > TI.config.map.maxMarkers) {
			start = 5;  // skip the first few which are often just prep shots
			total = TI.config.map.maxMarkers + 5;
