...
class Post {
	...
	getPhotos(callback) {
		if (this.photosLoaded && this.infoLoaded) {
			...
			flickr.getSet(post.id, sizes, !post.infoLoaded, (photos, info) => {
				if (photos !== null) {
					if (!post.infoLoaded) { post.addInfo(info); }

					post.photos = photos.photo;
					post.photoTagList = parsePhotoTags(post.photos);
					post.thumb = post.photos.find(p => p.isprimary);
...
function parsePhotoTags(photos) {
	...
	for (let p of photos) {
		...
		for (let t of tags) {
			 ...
			 * @type {String}
			 */
			let tagName = library.photoTags[t];
		...
		// update tag list with full tag names
		p.tags = photoTags.join(', ');
	}
