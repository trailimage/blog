...
Post.prototype.hasTags = function()
{
...
Post.prototype.addInfo = function(info)
{
	...
	this.description = this.originalDescription = info.description._content;
	// http://farm{farm-id}.staticflickr.com/{server-id}/{id}_{secret}_[mstzb].jpg
	this.thumbnail = format.string('http://farm{0}.staticflickr.com/{1}/{2}_{3}_s.jpg', info.farm, info.server, info.primary, info.secret);
 ...
 */
Post.prototype.getPhotos = function(sizes, callback)
{
	...
	flickr.getSet(p.id, sizes, !p.infoLoaded, function(photos, info)
	{
		...
		if (p.id != setting.flickr.poemSet && p.id != setting.flickr.featureSet)
		{
			...
			p.description = updateDescription(p.originalDescription, p.photos, p.video);
		}
		p.photosLoaded = true;
		callback();
	});
 ...
 * @returns {boolean}
 */
Post.prototype.isPartial = function() { return this.totalParts > 1; };

/**
 * Whether next post is part of the same series
 * @returns {boolean}
 */
Post.prototype.nextIsPart = function() { return this.next != null && this.title == this.next.title; };

/**
 * Whether previous post is part of the same series
 * @returns {boolean}
 */
Post.prototype.previousIsPart = function()
{
	return this.previous != null && this.title == this.previous.title;
};
...
/**
 * Load posts from request
 * @param req
 */
Post.fromRequest = function(req)
{
...
function updateDescription(description, photos, video)
{
	...
	{
		description = format.string('{0} (Includes {1} photos', description, photos.length);
