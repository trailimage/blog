...
const library = require('./library-old.js');
...
class Post {
	 ...
	 * @return {Post}
	 */
	static parse(flickrSet, chronological) {
		let p = new Post();
	...
	addInfo(info) {
		...
		this.description = this.longDescription = info.description._content.remove(lineBreak);
		// http://farm{farm-id}.staticflickr.com/{server-id}/{id}_{secret}_[mstzb].jpg
		// http://farm{{info.farm}}.static.flickr.com/{{info.server}}/{{info.primary}}_{{info.secret}}.jpg'
		let thumb = `http://farm${info.farm}.staticflickr.com/${info.server}/${info.primary}_${info.secret}`;
	...
	getPhotos(callback) {
		if (this.photosLoaded && this.infoLoaded) {
			...
			let flickr = require('../providers/flickr-old.js');
			let post = this;

			flickr.getSet(post.id, Photo.sizesForPost, !post.infoLoaded, (photos, info) => {
				if (photos !== null) {
					if (!post.infoLoaded) { post.addInfo(info); }

					post.photos = photos.photo.map((p, index) => Photo.parse(p, index));
					post.photoTagList = parsePhotoTags(post.photos);
					post.thumb = post.photos.find(p => p.primary);

					if (post.id != setting.flickr.photoSet.poetry && post.id != setting.flickr.photoSet.featured) {
						post.photoCoordinates = getPhotoCoordinates(post.photos);
						post.dateTaken = getDateTaken(post.photos);
 ...
 * @return {String}
 */
function parsePhotoTags(photos) {
	/**
	...
	for (let p of photos) {
		for (let i = 0; i < p.tags.length; i++) {
			 ...
			 * @type {String}
			 */
			let tagName = library.photoTags[p.tags[i]];
