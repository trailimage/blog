...
Post.prototype.addInfo = function(info)
{
	...
	var thumb = format.string('http://farm{0}.staticflickr.com/{1}/{2}_{3}', info.farm, info.server, info.primary, info.secret);
...
Post.prototype.getPhotos = function(callback)
{
	...
	else
	{
		...
		flickr.getSet(p.id, sizes, !p.infoLoaded, function(photos, info)
		{
			...
			{
				// flickr call failed
				if (p.retries++ < setting.flickr.maxRetries)
				{
					log.warn('%s retry %s for set %s', method, p.retries, p.id);
					setTimeout(function() { p.getPhotos(callback) }, setting.flickr.retryDelay);
				}
				else
				{
					log.error('%s failing after maximum retries for set %s', method, p.id);
				}
			}
			else
			{
				...
				if (p.retries > 0)
				{
					log.info('%s success after %s retries for set %s', method, p.retries, p.id);
					p.retries = 0;
				}
