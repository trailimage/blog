...
exports.remove = (key, p2, p3) => {
	...
	if (is.empty(key)) {
		...
		if ((is.array(p2) && p2.length === 0) || is.empty(p2)) {
			...
			remove([key].concat(p2), responder(p2, callback, replyType.none));
		}
	} else {
		remove(key, responder(key, callback, replyType.count));
	}
};
...
exports.exists = (key, p2, p3) => {
	if (p3 === undefined) {
		// p2 is the callback
		exists(key, responder(key, p2, replyType.bit));
	} else {
		// p2 is a field name, p3 is the callback
		memberExists(key, p2, responder(key, p3, replyType.bit));
	}
};
...
exports.keys = (key, callback) => {
	if (/[\?\*\[\]]/.test(key)) {
		...
		keys(key, responder(key, callback, replyType.raw));
	} else {
		// all fields of a hash key
		memberKeys(key, responder(key, callback, replyType.raw));
	}
};
...
function getValue(type, key, p2, p3) {
	if (p3 === undefined) {
		select(key, responder(key, p2, type));   // http://redis.io/commands/get
	} else {
		selectMember(key, p2, responder(key, p3, type));
	}
}
...
exports.add = (key, p2, p3, p4) => {
	if (p4 !== undefined || (p3 !== undefined && !(p3 instanceof Function))) {
		addMember(key, p2, normalize(p3), responder(key, p4, replyType.none));
	} else if (p3 !== undefined) {
		add(key, normalize(p2), responder(key, p3, replyType.okay));
	} else if (p2 !== undefined) {
		add(key, normalize(p2));
	}
};
