...
exports.tagSearch = function(tags, callback) {
	...
	signedCall(service, 'user_id', userID, r => { callback((r) ? r.photos.photo : null); }, {
...
exports.getSet = function(id, imageSizes, alsoGetInfo, callback) {
	...
	call(service, 'photoset_id', id, function(r) {
		...
		let photos = normalizePhotos(r.photoset);
...
exports.getAccessToken = function(token, verifier, callback) {
	oauth.getOAuthAccessToken(token, tokenSecret, verifier, function(error, accessToken, accessTokenSecret) {
		tokenSecret = null;
		if (error) { log.error(error); return; }
...
function call(method, idType, id, callback, args) {
	...
	let request = http.request(options, function(response) {
		...
		response.on('end', function() { sanitize(call, callArgs, result); });
	});
 ...
 * @param {object} args Original call arguments
 */
function retry(fn, args) {
	...
	if (retries.hasOwnProperty(key)) { count = ++retries[key]; } else {	retries[key] = count; }
	...
	if (count > setting.flickr.maxRetries) {
		...
		setTimeout(function() { fn.apply(scope, args); }, setting.flickr.retryDelay);
	}
}
...
function clearRetries(method, id) {
	...
	if (retries.hasOwnProperty(key) && retries[key] > 0) {
...
function signedCall(method, idType, id, callback, args) {
	...
	oauth.get(url, setting.flickr.token, setting.flickr.tokenSecret, function(error, data) {
		if (error) { log.error(error); retry(signedCall, callArgs); } else { sanitize(signedCall, callArgs, data); }
	});
}
...
function normalizePhotos(photos) {
	...
	for (let i = 0; i < photos.photo.length; i++) {
		...
		if (format.isEmpty(p.url_l)) {
			...
			let suffix = otherSize.replace('url', '');
