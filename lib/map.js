...
const index = { LAT: 0, LON: 1, ELEVATION: 2, TIME: 3, SPEED: 4 };
...
function trackFromGPX(node) {
   ...
   const track = Array.from(node.getElementsByTagName('trkseg'))
      ...
      .filter(segment => segment[0].length > 0)
      .map(segment => {
         totalDistance += length(segment);
         return simplify(segment.map(point => {
               ...
               return point.slice(0,3);  // remove time and speed from point
            }),
