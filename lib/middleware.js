...
function blockSpamReferers(req, res, next) {
    ...
    if (is_1.default.value(referer)) {
        checkBlackList(util_1.default.topDomain(referer)).then(spam => {
            if (spam) {
...
const getBlackList = () => cache_1.default.getItem(cacheKey).then(list => {
...
function enableStatusHelpers(req, res, next) {
    req.clientIP = () => {
        ...
        return util_1.default.IPv6(ipAddress);
    };
    res.notFound = () => {
