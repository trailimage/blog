...
const isStale = () => lastUpdate === 0 ||
    (config_1.default.referralSpam.updateFrequency > 0 &&
...
function enableViewCache(_req, res, next) {
...
function renderForType(res, slug, options) {
    if ([constants_1.mimeType.JSON, constants_1.mimeType.JSONP].indexOf(options.mimeType) >= 0 &&
        is_1.default.callable(options.generate)) {
    ...
    return (view, options, postProcess) => {
        ...
        res.render(view, options, (renderError, text) => {
            if (is_1.default.value(renderError)) {
                ...
                if (is_1.default.value(text)) {
                    cacheAndSend(res, text, slug, type);
                }
                else {
                    logger_1.default.error('renderTemplate(%s) returned no content', slug);
                    res.internalError();
                }
            }
        });
