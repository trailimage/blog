...
function serialize(linkData) {
    ...
    return JSON.stringify(linkData, (_key, value) => (value === null || value === 0 ? undefined : value));
}
function removeContext(linkData, context = null) {
    if (linkData !== undefined &&
        linkData !== null &&
        typeof linkData == is_1.default.type.OBJECT) {
        if (linkData.hasOwnProperty(contextField) &&
...
exports.default = {
    ...
    idField,
    fromPost(post) {
        const categoryTitle = Object.keys(post.categories).map(key => post.categories[key]);
        const schema = {
            author: owner(),
            name: post.title,
            headline: post.title,
