            ...
            switch (t) {
               ...
               case C.logTo.REDIS:
                  ...
                  tx.on('error', err => {
                     ...
                     _provider.remove(tx);
                     if (!is.defined(_provider.transports, C.logTo.CONSOLE)) {
                        _provider.add(new winston.transports.Console());
                     }
...
function parseLogs(results) {
   ...
   if (is.defined(results,'redis')) {
      ...
      for (let r of results.redis) {
         ...
         if (config.isProduction) { d = new Date(d.getTime() + (config.timezone * C.time.HOUR)); }
         let h = d.getHours();
         ...
         if (!sameDay(day, d)) {
            day = d;
            dayKey = format.string('{0}, {1} {2}', C.weekday[d.getDay()], C.month[d.getMonth()], d.getDate());
            grouped[dayKey] = [];
         }
