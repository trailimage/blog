...
const winston_redis_1 = require("winston-redis");
let queryable = false;
const level = {
    DEBUG: 'debug',
    INFO: 'info',
            ...
            transports: config_1.default.log.targets.map(t => {
                switch (t) {
                    ...
                    case constants_1.logTo.REDIS:
                        ...
                        const tx = new winston_redis_1.Redis({
                            host: url.hostname,
                            port: url.port,
...
function parseLogs(results) {
    ...
    if (is_1.default.defined(results, 'redis')) {
        ...
        for (const r of results.redis) {
            _1.default.logMessage(r, 'message');
            const d = new Date(r.timestamp);
            const h = d.getHours();
            r.timestamp = _1.default.format('{0}:{1}:{2}.{3} {4}', h > 12 ? h - 12 : h, _1.default.number.pad(d.getMinutes(), 2), _1.default.number.pad(d.getSeconds(), 2), _1.default.number.pad(d.getMilliseconds(), 3), h >= 12 ? 'PM' : 'AM');
            if (!sameDay(day, d)) {
                day = d;
                dayKey = _1.default.format('{0}, {1} {2}', constants_1.weekday[d.getDay()], constants_1.month[d.getMonth()], d.getDate());
                grouped[dayKey] = [];
            }
