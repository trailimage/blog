...
function buildPostPhoto(p, index) {
   return {
      id: p.id,
      index: index + 1,
      sourceUrl: linkBase + p.pathalias + '/' + p.id,
      title: p.title,
      description: p.description._content,
      // tag slugs are later updated to proper names
      tags: is.empty(p.tags) ? [] : p.tags.split(' '),
      dateTaken: format.parseDate(p.datetaken),
      latitude: parseFloat(p.latitude),
      longitude: parseFloat(p.longitude),
      primary: (parseInt(p.isprimary) == 1),
      // whether taken date is an outlier compared to other photos in the same post
      // http://www.wikihow.com/Calculate-Outliers
      outlierDate: false,
      size: {
         preview: buildPhotoSize(p, this.sizeField.preview),
         normal: buildPhotoSize(p, this.sizeField.normal),
         big: buildPhotoSize(p, this.sizeField.big)
      },
...
// Flickr makes each photo available in a variety of sizes
