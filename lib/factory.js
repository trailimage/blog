...
function parseCollections([collections, tags]) {
   ...
   for (let p of library.posts) { buildPostInfo(p); }
 ...
 * @returns {Object}
 */
function subcategory(slug) { return this.subcategories.find(t => t.title === slug || t.slug === slug); };
 ...
 * @returns {Boolean}
 */
function hasSubcategory(name) { return this.subcategory(name) !== undefined; }
...
function addSubcategory(subcat) {
   if (subcat !== null) {
      ...
      subcat.slug = this.slug + '/' + subcat.slug;
...
function buildPostInfo(post) {
   if (post.infoLoaded) { return; }

   flickr.getSetInfo(post.id).then(setInfo => {
...
function postName() {
   ...
   return p.title + ((p.isPartial) ? Post.subtitleSeparator + ' ' + p.subTitle : '');
};
...
function addPostPhotos(list) {
   ...
   if (this.photos.length > 0) {
      ...
      serializePhotoCoordinates(this);
