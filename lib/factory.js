...
const BLOG_JSON_KEY = 'blogJsonKey';
...
function addPostToLibrary(p) {
   ...
   if (p.chronological && library.posts.length > 1) {
      const next = library.posts[library.posts.length - 2];
 ...
 * @returns {Promise}
 */
function ensurePostLoaded() { return Promise.all([this.getInfo(), this.getPhotos()]); }
...
/**
 * Load all map photos for all posts
 * @returns {Promise.<ViewCacheItem>}
 */
...
/**
 * Get GeoJSON for post. If post has no track then GPX will only include photo
 * markers.
 * @param {string} postKey
 * @returns {Promise.<ViewCacheItem>}
 */
 ...
 */
const mapPhotoFeatures = geo => new Promise(resolve => { addPhotoFeature(geo, resolve); });
...
const mapPostPhotoFeatures = (post, geo) => new Promise(resolve => {
   ...
   addPostPhotoFeature(post, geo, resolve);
});
 ...
 */
function addPhotoFeature(geo, resolve) {
 ...
 * @param {function} resolve
 */
function addPostPhotoFeature(post, geo, resolve) {
   post.getPhotos().then(photos => {
      ...
      if (post.nextIsPart) {
         addPostPhotoFeature(post.next, geo, resolve);
      } else {
         resolve(geo);
      }
   });
