 ...
 * @return {Promise}
 */
function makeLibrary(reload = true) {
   if (reload && library.loaded) { library.empty(); }
   return Promise.all([flickr.getCollections(), flickr.getAllPhotoTags()]).then(parseCollections);
}
...
function parseCollections([collections, tags]) {
   library.photoTags = is.value(tags) ? tags : {};
   for (let c of collections) { makeCategory(c, true); }
   correlatePosts();
   library.loaded = true;
   // cache raw Flickr response
   cache.enqueue(collections);
   log.info('Loaded %d photo posts from Flickr: beginning detail retrieval', library.posts.length);
...
/**
 * Add Flickr collection to library singleton
 * @param {Object} collection
 * @param {Boolean} root Whether a root level collection
 */
function makeCategory(collection, root = false) {
   ...
   if (root) { library.tags[category.title] = category; }
   ...
   if (is.array(collection.set) && collection.set.length > 0) {
      ...
      for (let s of collection.set) {
         if (exclude.indexOf(s.id) == -1) {
            ...
            p.categories[category.slug] = category.title;
   ...
   if (collection.collection) {
      ...
      collection.collection.forEach(c => { addSubcategory(category, makeCategory(c)); });
   }
...
function addSubcategory(category, subcat) {
   if (subcat !== null) {
      let oldSlug = subcat.slug;

      subcat.slug = category.slug + '/' + subcat.slug;
      category.subcategories.push(subcat);

      // update posts that reference the tag by its old name
      for (let p of subcat.posts) {
         delete p.categories[oldSlug];
         p.categories[subcat.slug] = subcat.title;
      }
   }
}
...
//       Promise.all([flickr.getSetInfo(p.id), flickr.getSetPhotos(p.id)]).then(s => {
//
//       });
//
//       flickr.getSetInfo(p.id).then(s => {
//          if (post === null) {
