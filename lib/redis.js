...
function makeHandler(key, type = dataType.NONE, resolve, reject) {
   ...
   const answer = (actual, expected) => {
      if (expected === undefined || actual === expected) {
         // allow NULL as a valid response if nothing expected
         resolve(actual);
      } else {
 ...
 */
function removeMember(key, memberKey, callback) {
   client.hdel([key].concat(memberKey), callback);
}
   ...
   remove: (key, hashKey) => new Promise((resolve, reject) => {
      ...
      //    reject('Attempt to delete "' + key + '" field from disconnected cache');
         ...
         if ((is.array(hashKey) && hashKey.length === 0) || is.empty(hashKey)) {
            ...
            removeMember(key, hashKey, makeHandler(key, dataType.NONE, resolve, reject));
         }
      } else {
         const handler = makeHandler(key, dataType.COUNT, resolve, reject);

         if (is.array(key)) {
            client.hdel(key, handler);
         } else {
