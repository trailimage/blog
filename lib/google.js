 ...
 */
function loadGPX(post, stream) {
   const noCache = () => 0;
   return (config.cache.map)
      ? cache.map.item(post.key)
         .then(geo => is.value(geo) ? geo : noCache())
         .catch(err => {
            log.error('Error getting GPX "%s" from Google Drive: %j', post.title, id, err);
            return noCache();
         })
      : noCache();
}
...
const downloadFile = (fileId, post, res) => verifyToken().then(()=> {
   ...
   return new Promise((resolve, reject) => {
      if (is.value(res)) {
         ...
         drive().files.get(options).on('error', reject).pipe(res);
      } else {
         // capture file contents
         drive().files
            .get(options, (err, response, body) => {
               if (is.value(err)) { reject(err); } else { resolve(body); }
            })
            .on('error', reject)
      }
   });
