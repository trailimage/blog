 ...
 * @extends TI.Map.Feature
 * @see http://geojson.org/geojson-spec.html'
 * @alias TI.Map.Track
 */
class Track extends TI.Map.Feature {
	/**
	 * @namespace TI.Map.Track.parse
	 * @param {Node|Element} node
	 * @returns Track|TI.Map.Track
	 */
	static parse(node) {
		let t = new Track();
		...
		let total = 0;
		let dim = TI.Map.Location;

		for (let i = 0; i < segments.length; i++) {
			// NodeList is not ES6 iterable
			track.push(TI.Map.Line.parse(segments[i], 'trkpt'));
		}
		...
		stat.duration = (lastPoint[dim.time] - firstPoint[dim.time]) / TI.enum.time.hour;

		// combined max and average speeds for track segments
		for (let line of track) {
			for (let point of line) {
				let speed = point[dim.speed];
			...
			stat.distance += TI.Map.Line.length(line);
			line = simplify(line);
		}
		...
		let geoType = TI.Map.Type.multiLine;
		let coords = track;

		if (track.length === 1) {
			geoType = TI.Map.Type.line;
			coords = track[0];
		}

		t.properties = GPX.properties(node, stat);
		t.geometry = new TI.Map.Geometry(geoType, coords);
	 ...
	 * @param {TI.Post} post
	 * @param {function(TI.Map.Track|Track)} callback
...
function simplify(points) {
	...
	let tolerance = config.map.maxDeviationFeet / TI.enum.distance.equator;
	let first = 0;
	let last = len - 1;
	let stack = [];
	let newPoints = [];
...
function pointLineDistance(p, p1, p2) {
	...
	const xi = TI.Map.Location.longitude;
	// y index in the point object
	const yi = TI.Map.Location.latitude;
	let x = p1[xi];
	let y = p1[yi];
	let dx = p2[xi] - x;
	let dy = p2[yi] - y;
