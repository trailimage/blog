...
class Track extends GeoFeature {
	...
	static parse(node) {
		...
		for (let i = 0; i < segments.length; i++) {
			...
			track.push(Map.Line.parse(segments[i], 'trkpt'));
		}
		...
		stat.duration = (lastPoint[Map.Location.time] - firstPoint[Map.Location.time]) / Enum.time.hour;

		// combined max and average speeds for track segments
		for (let line of track) {
			for (let point of line) {
				let speed = point[Map.Location.speed];
			...
			stat.distance += Map.Line.length(line);
			line = simplify(line);
		}
		...
		t.geometry = new Map.Geometry(geoType, coords);
