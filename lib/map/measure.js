...
function pointLineDistance(p, p1, p2) {
    ...
    if (Δx !== 0 || Δy !== 0) {
        const t = ((p[_1.default.LON] - x) * Δx + (p[_1.default.LAT] - y) * Δy) /
            (Δx * Δx + Δy * Δy);
        if (t > 1) {
            x = p2[_1.default.LON];
            y = p2[_1.default.LAT];
...
function centroid(points) {
    ...
    if (count == 1) {
        return { lon: points[0][_1.default.LON], lat: points[0][_1.default.LAT] };
    }
    ...
    let z = 0;
    points.forEach(p => {
        const radLat = toRadians(p[_1.default.LAT]);
        const radLon = toRadians(p[_1.default.LON]);
