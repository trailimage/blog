...
const logType = { REDIS: 1, CONSOLE: 2 };
const flickrSize = {
   thumbnail:  'url_t',
   square75:	'url_sq',
   square150:  'url_q',
...
module.exports = {
   ...
   // Whether to use Persona based wwwhisper authentication (https://devcenter.heroku.com/articles/wwwhisper)
   usePersona: process.env['WWWHISPER_DISABLE'] !== '1',
   cacheOutput: isProduction,
   timezone: -6,
   repoUrl: null,
   owner,
   site: {
   ...
   log: {
      ...
      targets: isProduction ? [ logType.REDIS ] : [ logType.CONSOLE ],
      save: isProduction
   },
   style: {
      icon: {
   ...
   cacheDuration: e.time.day * 2,
   retryDelay: e.time.second * 30,
   referralSpam: {
      updateFrequency: 0,
   ...
   flickr: {
      ...
      size: flickrSize,
      photoSize: {
         post: [ flickrSize.small320, flickrSize.medium800, flickrSize.medium640, flickrSize.large1024, flickrSize.large1600, flickrSize.large2048 ],
         map: [ flickrSize.small320 ],
         search: [ flickrSize.square150 ]
      },
