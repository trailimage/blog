...
function call(method, idType, id, options = {}) {
   ...
   return new Promise((resolve, reject) => {
      ...
      const attempt = options.sign
         ? () => oauth.get(http.url, token.access, token.secret, (error, body) => {
              handle(error, body, attempt);
           })
         : () => request(http, (error, response, body) => { handle(error, body, attempt); });
...
module.exports = {
   getCollections: () => call('collections.getTree', type.USER, config.flickr.userID, { value: r => r.collections.collection }),
    ...
    */
   photoSearch: tags => call('photos.search', type.USER, config.flickr.userID, {
      args: {
