...
module.exports = () => {
	return (req, res, next) => {
		...
		res.notFound = () => {
			...
			res.render(template.page.notFound, { title: 'Page Not Found', setting: setting });
		};
...
function makeRenderer(res, slug, mimeType) {
	return (view, options, postProcess) => {
		...
		if (is.empty(options.description)) { options.description = setting.description; }
		options.setting = setting;

		res.render(view, options, (renderError, text) => {
			if (renderError) {
				log.error('Rendering “%s”: %j', view, renderError);
