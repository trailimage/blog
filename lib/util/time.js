...
function toLogTime(text) {
    ...
    return text_1.format('{0}/{1} {2} {3}:{4}:{5}.{6}', d.getMonth() + 1, d.getDate(), utility_1.weekday[d.getDay()], exports.hourOfDay(d.getHours()), utility_1.leadingZeros(d.getMinutes(), 2), utility_1.leadingZeros(d.getSeconds(), 2), utility_1.leadingZeros(d.getMilliseconds(), 3));
}
exports.toLogTime = toLogTime;
exports.timeZoneOffset = (date = new Date()) => config_1.default.timeZone + (utility_1.inDaylightSavings(date) ? 1 : 0);
function parseDate(text) {
    ...
    if (utility_1.inDaylightSavings(d)) {
...
function fromTimeStamp(timestamp) {
    if (is_1.default.date(timestamp)) {
        ...
        timestamp = utility_1.parseNumber(timestamp);
    }
...
function hoursAndMinutes(hours) {
    ...
    const m = hours - h;
    return h + ':' + utility_1.leadingZeros(Math.round(60 * m), 2);
}
