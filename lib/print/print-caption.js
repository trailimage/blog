...
class PrintCaption extends PrintArea {
	 ...
	 */
	fullWidth() {
		this.left = this.book.textMargin;
	...
	_calculateHeight() {
		...
		this.height = this.book.pdf.heightOfString(this.text, { width: this.widthInches }) + margin;
	}
	...
	render(callback) {
		...
		this.book.withTextFont().text(this.text, margin, null, { width: this.widthInches });
		callback();
	}
}
