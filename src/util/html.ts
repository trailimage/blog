 ...
 */
export const shortenLinkText = (text:string) => text.replace(re.tag.linkToUrl, (match, protocol, url:string) => {
 ...
 */
export const fraction = (text:string) => text.replace(/(\d+)\/(\d+)/, '<sup>$1</sup>&frasl;<sub>$2</sub>');

/**
 * Stylize punctuation
 */
export const typography = (text:string) => is.empty(text) ? '' : text
...
export function postModeIcon(categories:string[]|{[key:string]:Category}):string {
 ...
 */
export const characterEntities = (text:string) => text.replace(
   /[\u00A0-\u2666<>\&]/g,
