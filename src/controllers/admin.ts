...
function home(req: Blog.Request, res: Blog.Response) {
   ...
   Promise.all([
      ...
      apiKeys = is.array(apiKeys)
         ? apiKeys.map(j => j.replace(cache.api.prefix, ''))
         : [];
 ...
 */
function updateLibrary(_req: Blog.Request, res: Blog.Response) {
   return cache.api
      .remove(flickr.cache.keysForLibrary)
      .then(() =>
         library.load(false).then(() => {
...
function deleteViewCache(req: Blog.Request, res: Blog.Response) {
   ...
   if (apiHashKeys.length > 0) {
      ...
      removals.push(cache.api.remove(flickr.cache.keysForPost, apiHashKeys));
 ...
 */
function deleteApiCache(req: Blog.Request, res: Blog.Response) {
   const keys = req.body.selected;
